...
if __name__ == "__main__":
    ...
    disruptions_saved = {} # {5:[percentage of disruptions saved]}

    # intermediate spectrum occu lists to hold aggregate values over multiple runs
    spectrum_occupancy_intermed = {} # {5:([percentage of occupied spectrum],[times]), 10:([],[]), 15:([],[]), 20:([],[])}
    ground_spectrum_occupancy_intermed = {} # {5:([percentage of occupied ground spectrum],[times]), 10:([],[]), 15:([],[]), 20:([],[])}
    sat_spectrum_occupancy_intermed = {}
    gs_spectrum_occupancy_intermed = {}
    disruptions_saved_intermed = {}

    # initializing intermediate spectrum occupancy lists
    ...
    for j in range(no_of_runs):
        print("run:",j+1)
        # summing the results over multiple runs
        for i in range(len(requests)):
            times_list = [0 for i in range(num_of_iter+1)]
            list_of_saved_disrup, (spectrum_occu_for_one_run, times_list), (ground_spectrum_occ_for_one_run, times_list), (sat_spectrum_occ_for_one_run, times_list),  (gs_spectrum_occ_for_one_run, times_list) = run_simulation(G, num_of_iter, requests[i])
    ...
    print('disruptions_saved', disruptions_saved)
